    ...
    Scaffold(
        topBar = {
            TopAppBar(
                title = { Text("Turls") },
                actions = {
                    if (state.engagements.isNotEmpty()) {
                        TextButton(
                            onClick = { viewModel.handleIntent(TurlsHistoryIntent.ClearAll) }
                        ) {
                            Text("Clear")
                        }
                    }
        ...
        Box(
            ...
            if (state.engagements.isEmpty()) {
                EmptyContent(
                    title = "Looks like you don't have any Turls yet.",
                    description = "Start listen radio and Turls will appear here."
                )
            } else {
                EngagementsList(
...
private fun EngagementsList(
    engagements: List<HistoryEngagementEntity>,
    stations: List<Station>,
    onItemClick: (HistoryEngagementEntity) -> Unit,
    onDelete: (HistoryEngagementEntity) -> Unit
) {
    LazyColumn(
...
private fun EngagementCard(
    engagement: HistoryEngagementEntity,
    station: Station?,
    onClick: () -> Unit
) {
    Card(
